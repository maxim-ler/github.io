	...
	$('.slider2 .slick-next').on('click', function(e) {
		if ($(this).text() == 'Узнать стоимость') {
			if($('.slider2 .phone').val() == '') {
				...
				$name = $('.slider2 input.name').val();
				$phone = $('.slider2 .phone').val();
				$email = $('.slider2 .email').val();
				$area = $('.slider2 .area').val();
				$address = $('.slider2 .address').val();
				$count = $('.slider2 .count').val();



				if (!$('.slider2 p').hasClass('append2')) {
					$('.slider2').append('<p style="margin-top: 0px; background: #563D86; padding: 20px; font-size: 24px; padding-bottom: 20px; color: #fff;" class="append2">Спасибо! Данные успешно отправлены.</p>');
					$('.slider2 input').val('');
				}
				...
				$.ajax({
					...
					// $(this).find("input").val("");
					// $('.form-reg .form').find("input").val("");
					// $('.form-reg2 form').find("input").val("");
