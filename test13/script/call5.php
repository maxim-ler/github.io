...
$zapros = mysql_real_escape_string(strip_tags(trim($_POST['zapros'])));
...
<script>
$("#content_form5").html('<div style=\"text-align:center;padding:20px 30px 10px 30px;\"><span style=\"font-size:28px;font-weight:bold;font-family:Tahoma;\">Наш менеджер свяжется с вами в течении 15 минут</span><br /><span style=\"font-size:12px;font-weight:bold;font-family:Tahoma;margin-top:20px;\">В рабочее время компании:<br />Будние дни с 7:00 до 16:00 по московскому времени</span></div><div style=\"text-align:center;padding-bottom:20px;\"><center><a href=\"javascript:close_pop_up(\'#pop-up3\');\"><input class=\"form_button"\ type=\"submit\" value=\"Закрыть окно\"></a></center></div>');
</script>
...
<?php
	...
	$textsms = "JCB.Запрос КП.Тел:".$phone;
	...
	mysql_query("INSERT INTO click_zayavka (ip, site_name, city, keyword, full_key, ip_addr, client_phone, client_name, type, client_mail, client_kp) VALUE ('".$ip."', 'JCB', '".$city."', '".$keyword."', '".$full_key."', '".$addr."', '".$phone."', '".$name."', 3, '".$mail."', '".$zapros."') ") or die (mysql_error());
